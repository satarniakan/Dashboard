...
public class TestDataSeederService : ITestDataSeederService
{
    ...
    public async Task SeedAsync(string? userId)
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            var invoiceTotal = products[(i + 2) % 10].Price * 2;
            var installmentAmount = Math.Round(invoiceTotal / 3, 0);
            ...
            await _installmentService.CreateInstallmentPlanAsync(new CreateInstallmentPlanDto
            {
                ...
                {
                    new(DateTime.UtcNow.AddDays(-10), installmentAmount), // این یکی عقب‌افتاده است
                    new(DateTime.UtcNow.AddDays(20), installmentAmount),
                    new(DateTime.UtcNow.AddDays(50), installmentAmount)
                }
            }, userId);
        }
